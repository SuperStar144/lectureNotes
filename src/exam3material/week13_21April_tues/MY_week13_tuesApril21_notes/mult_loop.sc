...
def mult(x: Z, y: Z): Z = {
  ...
  Deduce(
    ...
    4 ( i == 0 ) by Premise,
    5 ( y >= 0 ) by Premise,
    6 ( i <= y ) by Algebra*(4,5)
    //want: total == x*i
  )
  //should be able to assert both invariants before loop begins
  assert(total == x*i)
  assert(i <= y)
  ...
  while (i < y) {
    ...
    Deduce(
      ...
      6 ( y >= 0 ) by Premise,
      7 ( Old(i) < y ) by Premise, //loop condition is true
      8 ( i - 1 < y ) by Algebra*(1,7),
      9 ( i <= y ) by Algebra*(1,6,7,8) //proves 2nd invariant
    )
  ...
  Deduce(
    ...
    2 ( i <= y ) by Premise, //2nd invariant condition is true after loop
    3 ( i >= y ) by Algebra*(1),

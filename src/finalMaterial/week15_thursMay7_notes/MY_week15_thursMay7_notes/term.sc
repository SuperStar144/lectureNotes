...
Inductive step. We assume the inductive hypothesis - that multRec terminates for some fixed y= k >= 0 where k 
is an integer.

We must show that multRec terminates when our second parameter (y) is k + 1. We would go into the else because
k+1 must be at least 1. There, we make a recursive call, passing y-1 (or k+1- = k) as the second parameter. 
(Really, calling multRec(x,k)). By our inductive hypothesis, that recursive call terminates. 
